export default `
    ...
    const float tau = 2.0 * 3.1415926535897932384626433832795;
    ...
    float plancksLaw(float wavelength, float temperature) {
    ...
    const vec3[441] radiationSpectrum = vec3[441](
    ...
    vec3 blackbody(float temperature, float shift) {
        vec3 luminocity = vec3(0);
        
        for (int i = 0; i < 440; i++) {
            vec3 radiationColor = radiationSpectrum[i];
            luminocity += xyzToRGB(radiationColor) * plancksLaw(float(i + 390) * shift, temperature);
        }
    ...
    // Not sure where I got this noise function from
    float noise(vec3 x) {
        vec3 p = floor(x);
        vec3 f = fract(x);
        f = f * f * (3.0 - 2.0 * f);
    ...
    vec3 rayMarch(vec3 origin, vec3 direction) {
        ...
        float h2 = pow(length(cross(origin, direction)), 2.0);

        for (int i = 0; i < uMaxIterations; i++) {   
            float distance = length(origin);         

            // Adjust direction based on gravitational lensing
            direction += -1.5 * h2 * origin / pow(pow(distance, 2.0), 2.5) * uStepSize;
            vec3 newPosition = origin + direction * uStepSize;

            // Hit event horizon
            if (distance <= uDiskInnerRadius - 1.5) { // Photons orbit is 1.5 smaller than the event horizon
                break;
            }

            distance = length(newPosition);     

            // Add volumetric depth variation
            float depthVariation = 0.0; // fbm(5.0 * newPosition, 5, 2.0, 1.0, 1.0);

            // Hit accretion disk
            if (distance >= uDiskInnerRadius && distance <= uDiskOuterRadius && origin.y * newPosition.y <= uStepSize + depthVariation) {
                float diskDensity = 1.0 - length(newPosition / vec3(uDiskOuterRadius, diskThickness, uDiskOuterRadius));
                // Account for the inner radius of the disk by scaling the density
                diskDensity *= smoothstep(uDiskInnerRadius, uDiskInnerRadius + 1.0, distance);
                
                // Inward flow of the disk matter
                vec3 uvw = vec3(atan(newPosition.z, abs(newPosition.x)) / tau - diskTwist / sqrt(distance), pow((distance - uDiskInnerRadius) / (uDiskOuterRadius - uDiskInnerRadius), 2.0) + 1.0 * (flowRate / tau) / (uDiskOuterRadius - uDiskInnerRadius), newPosition.y * 0.5 + 0.5) / 2.0;
                                
                // Scale the density to be higher in the center of the disk and lower at the edges
                // and add some noise to add some detail
                float densityVariation = fbm(2.0 * uvw, 5, 2.0, 1.0, 1.0);
                diskDensity *= densityVariation * inversesqrt(distance);
                
                // calculate depth of accretion disk relative to it's density
                float opticalDepth = uStepSize * 5.0 * diskDensity;
                // Absorption scales with density
                transmittance *= exp(-opticalDepth);

                // Doppler shift
                // https://physics.stackexchange.com/questions/685802/what-is-the-frequency-of-the-light-seen-by-an-observer-free-falling-into-a-black
                // shift = sqrt((c-v)/(c+v))
                // assuming c = 1
                // We calculate the vector of the velocity of the photon relative to the disk
                vec3 shiftVector = flowRate * cross(normalize(newPosition), vec3(0.0, 1.0, 0.0)); // we assume the flow is in the xz plane
                float velocity = dot(direction, shiftVector); // velocity of the photon in the direction of the accretion disk
                float dopplerShift = sqrt((1.0 - velocity) / (1.0 + velocity)); // Flip the sign to shift the wavelength to the red side
                
                float cameraDistance = length(uCameraPosition);

                // Gravitational shift
                // https://vixra.org/pdf/1704.0123v1.pdf
                // inversed by sqrt to "unshift" the doppler shift
                float gravitationalShift = sqrt((1.0 - 2.0 / distance) / (1.0 - 2.0 / cameraDistance)); // 1.0 / shift for more accurate doppler shift
                
                // Scale the temperature of the disk by the density and distance from the center
                // also add some noise to the temperature to add some variation in the matter
                float temperatureVariation = fbm(3.0 * uvw + 0.5, 4, 2.0, 1.0, 0.5);
                float temperature = uDiskTemperature * temperatureVariation * (4.5 * sqrt(diskDensity) * inversesqrt(distance));
                
                vec3 wavelength = blackbody(temperature, dopplerShift * gravitationalShift) * dopplerShift * gravitationalShift;
                color += transmittance * wavelength * opticalDepth;
            }
    ...
    void main() {
        // Scale the uv to the canvas resolution
